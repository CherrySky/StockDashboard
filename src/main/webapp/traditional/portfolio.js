...
function ApplicationModel(stompClient) {
  ...
  self.stockModel = ko.observable(new StockModel(stompClient));
...
function PortfolioModel() {
  ...
  self.updatePosition = function(position) {
    var portfolioRow = rowLookup[position.ticker];
    
    if (portfolioRow != null) {
    	rowLookup[position.ticker].shares(position.shares);
    } else {
    	var row = new PortfolioRow(position);
        self.rows.push(row);
        rowLookup[row.ticker] = row;
    }
...
function StockModel(stompClient) {
	var self = this;
	self.stockTicker = ko.observable();
	self.error = ko.observable();
	self.showAddStock = function() { self.showModal(); };
	self.showModal = function() {
		self.stockTicker('');
		$('#stock-dialog').modal();
	};
	
	$('#stock-dialog').on('shown', function() {
		var input = $('#stock-dialog input');
		input.focus();
		input.select();
	});
	...
	self.addStock = function() {
		...
		console.log(stock);
		stompClient.send("/app/addStock", {}, JSON.stringify(stock));
